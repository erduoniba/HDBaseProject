...
#define HDSafeLog(...) safeCollectionLog(__VA_ARGS__)
...
- (id)hd_objectAtIndex:(NSUInteger)index {
    ...
    if (index >= [self count]) {
        HDSafeLog(@"[%@ %@] index {%lu} beyond bounds [0...%lu]",
...
- (NSArray *)hd_arrayByAddingObject:(id)anObject {
    ...
    if (!anObject) {
        HDSafeLog(@"[%@ %@] arrayByAddingObject object is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return self;
    }
...
- (void)hd_addObject:(id)anObject {
    ...
    if (!anObject) {
        HDSafeLog(@"[%@ %@] addObject object is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
...
- (void)hd_insertObject:(id)anObject atIndex:(NSUInteger)index {
    ...
    if (!anObject) {
        HDSafeLog(@"[%@ %@] insertObject object is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
    ...
    if (index > [self count]) {
        HDSafeLog(@"[%@ %@] insertObject index {%lu} beyond bounds [0...%lu]",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd),
                  (unsigned long)index,
                  MAX((unsigned long)self.count - 1, 0));
...
- (void)hd_removeObjectAtIndex:(NSUInteger)index {
    ...
    if (index >= [self count]) {
        HDSafeLog(@"[%@ %@] removeObjectAtIndex index {%lu} beyond bounds [0...%lu]",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd),
                  (unsigned long)index,
                  MAX((unsigned long)self.count - 1, 0));
...
- (void)hd_replaceObjectAtIndex:(NSUInteger)index withObject:(id)anObject {
    ...
    if (index >= [self count]) {
        HDSafeLog(@"[%@ %@] replaceObjectAtIndex index {%lu} beyond bounds [0...%lu]",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd),
                  (unsigned long)index,
                  MAX((unsigned long)self.count - 1, 0));
    ...
    if (!anObject) {
        HDSafeLog(@"[%@ %@] replaceObjectAtIndex object is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
...
- (void)hd_removeObjectForKey:(id)aKey {
    ...
    if (!aKey) {
        HDSafeLog(@"[%@ %@] removeObjectForKey key is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
...
- (void)hd_setObject:(id)anObject forKey:(id <NSCopying>)aKey {
    ...
    if (!anObject) {
        HDSafeLog(@"[%@ %@] setObject:forKey object is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
    if (!aKey) {
        HDSafeLog(@"[%@ %@] setObject:forKey key is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
...
- (unichar)hd_characterAtIndex:(NSUInteger)index {
    ...
    if (index >= [self length]) {
        HDSafeLog(@"[%@ %@] characterAtIndex index {%lu} beyond bounds [0...%lu]",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd),
                  (unsigned long)index,
...
- (NSString *)hd_substringWithRange:(NSRange)range {
    ...
    if (range.location + range.length > self.length) {
        HDSafeLog(@"[%@ %@] substringWithRange range {%lu, %lu} beyond bounds [0...%lu]",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd),
...
- (void)hd_appendString:(NSString *)aString {
    ...
    if (!aString) {
        HDSafeLog(@"[%@ %@] appendString string is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
...
- (void)hd_appendFormat:(NSString *)format, ... NS_FORMAT_FUNCTION(1,2) {
    ...
    if (!format) {
        HDSafeLog(@"[%@ %@] appendFormat format is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
...
- (void)hd_setString:(NSString *)aString {
    if (!safeEnable) {
        return [self hd_setString:aString];
    }

    if (!aString) {
        HDSafeLog(@"[%@ %@] setString string is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
...
- (void)hd_insertString:(NSString *)aString atIndex:(NSUInteger)index {
    ...
    if (index > [self length]) {
        HDSafeLog(@"[%@ %@] insertString:atIndex index {%lu} beyond bounds [0...%lu]",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd),
                  (unsigned long)index,
                  MAX((unsigned long)self.length - 1, 0));
    ...
    if (!aString) {
        HDSafeLog(@"[%@ %@] insertString:atIndex string is nil",
                  NSStringFromClass([self class]),
                  NSStringFromSelector(_cmd));
        return;
    }
