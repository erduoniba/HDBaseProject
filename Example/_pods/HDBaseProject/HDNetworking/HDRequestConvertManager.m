...
NSString * const HDDNetworkCacheKeys = @"HDDNetworkCacheKeys";
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _cacheKeys = [self.cache objectForKey:HDDNetworkCacheKeys];
        if (!_cacheKeys) {
...
- (void)addCacheKey:(NSString *)cacheKey atRequestUrl:(NSString *)requestUrl {
    ...
    [self.cache setObject:self.cacheKeys forKey:HDDNetworkCacheKeys];
}
